...
One template covers **every** dataset. It carries the 59 Tinkhundla and all 11 value columns, extracted from the live `administrations` table on 2026-08-18, sorted by region then name.
...
|---|---|---|
| `administration_id` | **no** | The platform's key. Matched first |
| `inkhundla_name` | **no** | Cross-check against the id — a mismatch is an error, not a preference |
| `region` | **no** | Ignored on upload. Present so the sheet can be sorted to match a provider file |
| the 11 value columns | **yes** | Fill the ones you have. Leave the rest alone |
...
| Column | Unit / range | Usually comes from |
|---|---|---|
| `water_demand` | m³/year | DWA / JRBA permit registry |
| `cattle` | head | Ministry of Agriculture |
| `population` | people | WorldPop / CSO |
| `land_use_dvi_agri` | ratio, 0–1 | land-cover product |
| `ipc_phase` | phase, 1–5 | IPC Technical Working Group |
| `under_five` | people | CSO census |
| `elderly` | people | CSO census |
| `rainfed_cropland` | ha | land-cover product |
| `rangeland` | ha | land-cover product |
| `boreholes` | count | DWA WASH point inventory |
| `taps` | count | DWA WASH point inventory |
