...
suites green. Citizen-science block ships as an empty state pending WX-6.
...
| Weather Stations — **Citizen-science block** | *(no source yet — see G1)* | **Honest empty state for #146.** The block renders per the AC but with no data ("no citizen-science submission for this Inkhundla this month") until the CS platform + DIH ingestion land. It is a **real, planned source**, not a permanent gap: see [`../track-3/citizen-science-weather-ingestion.md`](../track-3/citizen-science-weather-ingestion.md) (WX-6). When that ships the block fetches `GET /weather/administrations/{id}/citizen-science?period=` and the empty state flips to real data with **no layout change**. Per-Inkhundla exact match, no fallback. |
