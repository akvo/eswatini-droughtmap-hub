...
class InsightsRegionGeoView(APIView):
    """The four administrative region polygons, as TopoJSON.

    Served straight from `source/`, not from a generated artefact: the file is
    3.5 KB and already in degrees, and the frontend decodes TopoJSON anyway.
    Its sibling below needs a build step only because that source is in
    Transverse Mercator metres.
    """
    ...
    @extend_schema(
        tags=["Insights"],
        summary="Get the administrative region topojson",
        responses={(200, "application/json"): OpenApiTypes.OBJECT},
    )
    def get(self, request, version=None):
        path = os.path.join(settings.BASE_DIR, REGION_TOPOJSON)
        if not os.path.exists(path):
            raise NotFound("Administrative region geometry is not available.")
...
class InsightsAgroEcoGeoView(APIView):
    """The agro-ecological region polygons, as WGS84 TopoJSON.

    Served from its own endpoint rather than riding on config.js: it is 173 KB
    ...
    def get(self, request, version=None):
        path = os.path.join(settings.BASE_DIR, AGRO_WGS84_TOPOJSON)
        if not os.path.exists(path):
